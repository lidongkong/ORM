...
(function($) {
    ...
    function addElementsViaJSON(options, selected) {
        ...
        $.getJSON(options.uri, function(json) {
			...
			if (json._items !== undefined && json._items !== null) {
				//alert( 'inside: ' + JSON.stringify(json._items));
				json = json._items;
			}
			
			//alert( 'out: ' + JSON.stringify(json));
	...
	function addElementsViaJSON_Old(options, selected) {
        var multipleTextFields = false;
