...
(function() {
  ...
  app.controller('MiswordController', function($scope, RepositoryFactory, resolveEntity) {
    ...
    $scope.dataRowsGridOptions = {
      ...
      columnDefs: [
        ...
        {
          field: '0',
          displayName: 'Misspelled Word'
        },
		{
          field: '1',
          displayName: 'Correct Word'
        },
        //{
    ...
    $scope.createRow = function(newKeyword) {
		alert(JSON.stringify(newKeyword));
      $scope.$broadcast('ngGridEventEndCellEdit');
      if (newKeyword.mWord.length > 0 && newKeyword.cWord.length > 0) {
